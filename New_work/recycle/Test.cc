...
/*--------------------------------------------- Checking for the XY coincidences on all the three layers and rejecting hits with no such coincidence ----------------------------------*/
  ...
  //-----------------------------------------------------------adding noise to the muon hits------------------------------------            
           for(int i = 0; i < num_noise_hits; i++)
          {  
             int rand_ind = rand->Integer(64512);
             tmp = mp_noise[rand_ind]; 
             x_c = tmp[0]/10.0;
             y_c = tmp[1]/10.0;
             z_c = tmp[2]/10.0;
             tmp.clear();
             
          //   cout<<"random ch id = "<<rand_ind<<endl;
...
/*           for(auto i = 0; i < ntheta_pos_x.size(); i++)
         ...
         */ 
         ...
         /*   f.cd();
          ...
          zoom_houghXZ->SetTitle(name_acc_xz2.c_str());
          zoom_houghXZ->GetXaxis()->SetLimits(new_theta_minXZ, new_theta_maxXZ);
          zoom_houghXZ->GetXaxis()->SetTitle("#theta position (degree)");
          zoom_houghXZ->GetYaxis()->SetTitle("#rho position (cm)");  
          zoom_houghXZ->SetStats(0);
          gROOT->ForceStyle();
          gStyle->SetPalette(55);
          zoom_houghXZ->Draw("COLZ");
      ...
      /*    cout<<"---------------thetaYZ--------------------------------"<<endl;
         ...
         /*   f.cd();
           ...
           */ 
           ...
           // c2->Divide(2,2);
            ...
            f.cd();
            std::string coord_name_canvas5 =  "Hough_simu"+ std::to_string(k);
            TCanvas *c5 = new TCanvas("c5",coord_name_canvas5.c_str());
            c5->SetCanvasSize(1800, 1200);
            c5->SetWindowSize(1800, 1000);
           // c2->Divide(2,2);
            c5->SetName(coord_name_canvas5.c_str());
          
            f.cd();
            std::string coord_name_canvas6 =  "Hough_simu"+ std::to_string(k);
            TCanvas *c6 = new TCanvas("c6",coord_name_canvas6.c_str());
            c6->SetCanvasSize(1800, 1200);
            c6->SetWindowSize(1800, 1000);
           // c2->Divide(2,2);
            c6->SetName(coord_name_canvas6.c_str());
       ...
       /*     c2->cd();
         ...
         */   
            ...
            grxz2->GetXaxis()->SetTitle("X pos of hits (cm)");
            grxz2->GetYaxis()->SetTitle("Z pos of hits (cm)");
