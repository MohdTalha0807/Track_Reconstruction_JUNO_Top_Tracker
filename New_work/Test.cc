...
{  //1
  ...
  // treeOut -> Branch("Evt_ID",&Ev_Id, "Ev_Id/I");
   ...
   int lim2=0;
   int noise_rate = 0;
   cout<<"Please insert the noise rate (kHz) : ";
   cin>>noise_rate;
...
/*---------------------------------------------------------------------------generating noise hits---------------------------------------------------*/
      
      double ac_time = 1e-7; // acquisition time = 100 nsec  ->  1e-7 sec
      int num_channels = 64512;
      int num_noise_hits = noise_rate * num_channels * ac_time * 1000;
      //float num_noise_hits2 = noise_rate * num_channels * ac_time;
      //cout<<"n noise = "<< num_noise_hits2 << endl;
      TRandom3  *rand = new TRandom3(0);
  
//----------------------------------------------------------------------------------------------------------------------------------------------   

    
      if(No_Channels==0 or k == 8057)continue;
      ...
      int number_row=7;
      lf_ch.clear();
      int leaf_channel_noise = 0, indx = 0;
     float x_c = 0.0, y_c = 0.0, z_c = 0.0;
        for(int i = 0; i < num_noise_hits; i++)
     {   int rand_ind = rand->Integer(64512);
          leaf_channel = mp_noise[rand_ind];  
          lf_ch.push_back(leaf_channel);
      ...
      { 
        ...
        {   indx = 0;
          for(int hit=0; hit < (No_Channels + num_noise_hits); hit++)
          { 
             if(hit < No_Channels)
            ...
            {  leaf_channel = lf_ch[indx];  
               ch_pe = 1.0; indx++;   
             }   

            if(ch_pe >= threshold_pe)
            { 
...
/*------------------------------------------------ Applying the Hough Transform --------------------------------------------------*/
 ...
 /*-------------------------------------------------------------------------------algorithm for isolating the peaks-----------------------------------------------------*/
        ...
        float thix, rhojx, thiy, rhojy, th1xz = 90.0, th2xz = 0.0, th1yz = 90.0, th2yz = 0.0, et = 2.0, e_zoom = 4.0, erhy = 60.0, rh1yz = 1200.0, rh2yz = 0.0, er = 80.0, erx = 20.0, ery = 20.0, rh1xz = 3000.0, rh2xz = 0.0;  
     ...
     /*   cout<<"------------------------------------------------------"<<endl;
         ...
         {  
           cout <<theta_pos_x[i]<<"\t"<<rho_pos_x[i]<<"size = "<<theta_pos_x.size()<<endl;            
         }
      */   
      ...
      { //5      
         ...
         { //    if(theta_pos_x[i] > 0)
          ...
          {    
             ...
             if((((temp[0] - et)  < thiXZ) && (thiXZ < (temp[0] + et))) && (((temp[1] - erx) < rhojXZ) && (rhojXZ < (temp[1] + erx))))
              {  
                 same_thrh_counter++;  
                 temp.clear();        
              }              
          ...
          if(siz1 < mp_th_rhxz.size())           
        {       temp.clear();
         for(auto itr =  mp_th_rhxz.begin(); itr !=  mp_th_rhxz.end(); ++itr)
              {       temp = itr -> second;
                      th_XZ.push_back(temp[0]);
                      rh_XZ.push_back(temp[1]);
                      temp.clear();
               }                      
...
/*           for(auto i = 0; i < ntheta_pos_x.size(); i++)
    ...
    //   cout<<"------------------------------------------------------"<<endl;
     ...
     /*            f.cd();
          ...
          c3->Clear();   */ 
           ...
           if(siz2 < mp_th_rhyz.size()) 
         {      temp.clear();
         for(auto itr =  mp_th_rhyz.begin(); itr !=  mp_th_rhyz.end(); ++itr)
              {       temp = itr -> second;
                      th_YZ.push_back(temp[0]);
                      rh_YZ.push_back(temp[1]);
                      temp.clear();
               }  
  ...
  /*     cout<<"theta size = "<<thetaYZ.size()<<endl;
      cout<<"---------------------------------------------"<<endl;
        
        for(int i = 0; i < thetaYZ.size(); i++)
        {
             cout<< thetaYZ[i] << "\t" << endl;
...
/*number of entries in TTree = 3563
3014	3188	3012	3188
-------------------------------------------------------------
efficiency in XZ Proj: 0.94542
efficiency in YZ Proj: 0.944793
-------------------------------------------------------------
*/
